...
class RandomCropInputAndOuput:
    def __init__(
        self,
        p: float,
        same_aug_for_all_pairs: bool,
        possible_num_cols_to_crop: list[int] = [1, 2],
        possible_num_rows_to_crop: list[int] = [1, 2],
    ):
        self.p = p
    ...
    ) -> Union[BoardPair, list[BoardPair], Riddle]:
        params = dict(cols_to_crop=self.cols_to_crop, rows_to_crop=self.rows_to_crop)
        func = cropInputAndOutput
        return p_and_same_aug_helper(
            input, func, self.p, self.same_aug_for_all_pairs, self.get_params, **params
        )
...
class RandomDoubleInputBoard:
    def __init__(
        ...
        p: float,
        same_aug_for_all_pairs: bool,
        possible_separations: list[int] = [-1, 1, 2],
        direction_type: Direction = Direction.both,
        random_z_index: bool = True,
    ):
        """
        Doubles the input board only, leaving the output board the same.
        possible_separations: list of possible separations
        separation if negative means some overlap between the two boards
        direction_type: the direction of concatnation.
        random_z_index: whether to randomize the z index of the overlap
        """
        self.p = p
        self.same_aug_for_all_pairs = same_aug_for_all_pairs
    ...
    def __call__(
        ...
        params = dict(
            possible_separations=self.possible_separations,
            direction_type=self.direction_type,
            random_z_index=self.random_z_index,
        )
        func = doubleInputBoard
        return p_and_same_aug_helper(
            input, func, self.p, self.same_aug_for_all_pairs, self.get_params, **params
        )
...
class RandomRotate:
    def __init__(
        ...
        p: float,
        same_aug_for_all_pairs: bool,
        number_of_90_degrees_rotations: int = 3,
    ):
        """
    ...
    def get_params(seed, number_of_90_degrees_rotations):
        ...
        assert (
            r >= 1 and r <= 3
        ), f"number_of_90_degrees_rotations must be between -1 and 3, not {r}"
    ...
    def __call__(
        ...
        params = dict(
            number_of_90_degrees_rotations=self.number_of_90_degrees_rotations
        )
        func = rotate
        return p_and_same_aug_helper(
            input, func, self.p, self.same_aug_for_all_pairs, self.get_params, **params
...
class RandomPad:
    def __init__(
        ...
        p: float,
        same_aug_for_all_pairs: bool,
        pad_sizes: list[int] = [1, 2],
        pad_values: list[int] = [0],
    ):
        """
...
class RandomSuperResolution:
    def __init__(
        ...
        p: float,
        same_aug_for_all_pairs: bool,
        super_resolution_factors: int = [2, 3],
        stretch_axis: list[Direction] = [Direction.both],
    ):
        """
    ...
    def __call__(
        ...
        params = dict(
            super_resolution_factors=self.super_resolution_factors,
