...
def same_aug_for_all_pairs_helper(
    input: Union[BoardPair, list[BoardPair], Riddle],
    ...
    if isinstance(input, list):
        return _same_aug_boardpair_list_helper(
            input,
            transformation_function,
            same_aug_for_all_pairs,
            get_params_method,
            params_in,
        )
    elif isinstance(input, BoardPair):
        assert (
            params_in is not None or get_params_method is not None
        ), "params_in or get_params_method must be provided"
        params = (
            get_params_method(seed=random.random()) if params_in is None else params_in
        )
        return transformation_function(input, *params)
    elif isinstance(input, Riddle):
        all_bps = [i for i in input.train] + [i for i in input.test]
        train_len = len(input.train)
        transformed_bps = _same_aug_boardpair_list_helper(
            all_bps,
            transformation_function,
            same_aug_for_all_pairs,
            get_params_method,
...
def _same_aug_boardpair_list_helper(
    input: list[BoardPair],
    transformation_function: Callable,
    same_aug_for_all_pairs: bool,
    get_params_method=None,
    params_in=None,
    ...
    if same_aug_for_all_pairs:
        ...
        assert (
            get_params_method is not None
        ), "get_params_method must be provided if same_aug_for_all_pairs is False"
        output = []
...
def p_and_same_aug_helper(input, func, p, same_aug_for_all_pairs, get_params, **params):
    if random.random() < p:
        ...
        return same_aug_for_all_pairs_helper(
            input, func, same_aug_for_all_pairs, get_params_method
        )
    else:
        return input
