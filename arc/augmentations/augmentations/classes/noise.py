...
class Noise(object):
    ...
    def get_params(seed, **kwargs):
        ...
        assert noise_level >= 0.0 and noise_level <= 1.0, f"noise_level must be a percentage (between 0 and 1)"
    ...
    def __call__(self, input:Union[BoardPair,list[BoardPair], Riddle])->Union[BoardPair,list[BoardPair]]:
        ...
        assert type(input) == Riddle, "Please input a Riddle"
        ...
        if random.random() < self.p:
            get_params_method = functools.partial(self.get_params,noise_level=self.noise_level,noise_size=self.noise_size)
