...
def doubleBoard(
    ...
    """
    Takes a Board and returns a new Board with the input board doubled.
    If is_horizontal_cat = True, the board is horizontally
    concatenated otherwise concatenation is vertical.
    Separation is the number of pixels between the two boards.
    Function supports negative separation values:
    one board is cropped to accomodate the other.
    if separation is negative, the z_index_of_original is
    used to determine which board lays ontop of the other.
    """
...
def rotate(board_pair: BoardPair, num_rotations: int = 0) -> BoardPair:
    """
    Takes a Board and returns a Board rotated 90 degrees `num_rotations` times
    """
...
def reflect(
    ...
    """
    Takes a Board and returns a Board reflected over: x-axis, y-axis, xy-axes, no axes
    """
